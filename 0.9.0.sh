...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "http://tewui4r0x.bkt.gdipper.com/latest.json" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}检测 XrayR 版本失败，可能是超出 Github API 限制，请稍后再试，或手动指定 XrayR 版本安装${plain}"
            exit 1
        fi
        echo -e "检测到 XrayR 最新版本：${last_version}，开始安装"
        wget -q -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux.zip http://tewui4r0x.bkt.gdipper.com/0.9.0.zip
        if [[ $? -ne 0 ]]; then
        ...
        url="http://tewui4r0x.bkt.gdipper.com/0.9.0.zip"
    ...
    file="http://tewui4r0x.bkt.gdipper.com/XrayR.service"
    ...
    curl -o /usr/bin/XrayR -Ls http://tewui4r0x.bkt.gdipper.com/XrayR
